...
const directoryVersion = "20260903";
...
${ordered.map((participant) => `      <a class="participant-thumb" href="participants/?v=${directoryVersion}#${escapeHtml(slug(participant))}" aria-label="View ${escapeHtml(participant.name)} in the participant directory" title="${escapeHtml(participant.name)}"><img src="${escapeHtml(imageSource(participant, "assets/people/"))}" alt="${escapeHtml(participant.name)}" loading="lazy" decoding="async" width="480" height="600"></a>`).join("\n")}
    </div>
  </div>
  <div class="participant-preview-action"><a class="button" href="participants/?v=${directoryVersion}">View participant directory <span aria-hidden="true">→</span></a><p>Photographs, affiliations and verified profile links</p></div>`;
